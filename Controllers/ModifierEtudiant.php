...
if ($_SERVER['REQUEST_METHOD'] === "PUT") {
    ...
    if (!empty($data->Matricule) && !empty($data->Nom)) {
        ...
        $result = $etudiant->ModifierEtudiant();
        if ($result) {
            http_response_code(201);
            echo json_encode(['message' => "Modification effectuée avec sucès"]);
        } else {
            http_response_code(503);
            echo json_encode(['message' => "Modification de l'étudiant a échoué"]);
        }
    } else {
